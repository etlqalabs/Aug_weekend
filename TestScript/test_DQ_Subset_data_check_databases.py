...
def test_compare_oracleToMysql_extraction(connect_to_oracle_source,connect_to_mysql_target):
    ...
    df_missing_record=df_mysql[~df_mysql['id'].isin(df_oracle['id'])]
    print(df_missing_record)
    assert df_missing_record.empty," some extra records in my target - please verify"
